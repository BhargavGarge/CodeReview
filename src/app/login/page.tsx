"use client";

import { createClient } from "@/lib/supabase/client";
import { useSearchParams } from "next/navigation";
import { Github } from "lucide-react";
import { Suspense } from "react";

const logoPath =
  "M1.04356 6.35771L13.6437 0.666504L26.2438 6.35771V20.0443L13.6437 25.7355L1.04356 20.0443V6.35771Z";

function LoginContent() {
  const searchParams = useSearchParams();
  const mode = searchParams?.get("mode"); // 'signup' for "Get Started" flow
  const error = searchParams?.get("error");
  const next = searchParams?.get("next") ?? "/dashboard";

  const isSignUp = mode === "signup";

  async function handleOAuth(provider: "github" | "google") {
    const supabase = createClient();
    await supabase.auth.signInWithOAuth({
      provider,
      options: {
        redirectTo: `${window.location.origin}/auth/callback?next=${encodeURIComponent(next)}`,
      },
    });
  }

  return (
    <main className="min-h-screen bg-[#010101] flex items-center justify-center px-4">
      {/* Ambient glow */}
      <div className="pointer-events-none fixed -left-24 top-20 h-96 w-96 rounded-full bg-[#7b39fc]/15 blur-[120px]" />
      <div className="pointer-events-none fixed -right-24 bottom-16 h-96 w-96 rounded-full bg-[#c967e8]/12 blur-[140px]" />

      <div className="relative z-10 w-full max-w-md">
        {/* Logo */}
        <a
          href="/"
          className="mb-10 flex items-center justify-center gap-3"
          aria-label="Go to homepage"
        >
          <svg
            width="28"
            height="28"
            viewBox="0 0 28 28"
            fill="none"
            aria-hidden="true"
          >
            <path d={logoPath} fill="#FFFFFF" />
          </svg>
          <span className="text-[14px] font-semibold tracking-[0.08em] text-white/90">
            CODE REVIEW
          </span>
        </a>

        {/* Card */}
        <div className="rounded-2xl border border-white/10 bg-[#0d0d14] p-8">
          <h1 className="mb-2 text-center text-2xl font-semibold text-white">
            {isSignUp ? "Create your account" : "Welcome back"}
          </h1>
          <p className="mb-8 text-center text-sm text-white/50">
            {isSignUp
              ? "Join developers shipping better code, faster."
              : "Sign in to your CodeReview.live account."}
          </p>

          {error && (
            <div className="mb-6 rounded-lg border border-red-500/20 bg-red-500/10 p-3 text-center text-sm text-red-400">
              Authentication failed. Please try again.
            </div>
          )}

          <div className="flex flex-col gap-3">
            <button
              type="button"
              onClick={() => handleOAuth("github")}
              className="flex w-full items-center justify-center gap-3 rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm font-medium text-white transition-colors hover:bg-white/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#7b39fc]"
            >
              <Github size={18} />
              Continue with GitHub
            </button>

            <button
              type="button"
              onClick={() => handleOAuth("google")}
              className="flex w-full items-center justify-center gap-3 rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm font-medium text-white transition-colors hover:bg-white/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#7b39fc]"
            >
              <GoogleIcon />
              Continue with Google
            </button>
          </div>

          <p className="mt-6 text-center text-xs text-white/30">
            By continuing, you agree to our Terms of Service and Privacy Policy.
          </p>
        </div>

        <p className="mt-6 text-center text-sm text-white/40">
          {isSignUp ? (
            <>
              Already have an account?{" "}
              <a href="/login" className="text-[#7b39fc] hover:underline">
                Sign in
              </a>
            </>
          ) : (
            <>
              New here?{" "}
              <a
                href="/login?mode=signup"
                className="text-[#7b39fc] hover:underline"
              >
                Get started free
              </a>
            </>
          )}
        </p>
      </div>
    </main>
  );
}

// Suspense required because useSearchParams() opts the page into client rendering
export default function LoginPage() {
  return (
    <Suspense>
      <LoginContent />
    </Suspense>
  );
}

function GoogleIcon() {
  return (
    <svg
      width="18"
      height="18"
      viewBox="0 0 18 18"
      fill="none"
      aria-hidden="true"
    >
      <path
        d="M17.64 9.205c0-.639-.057-1.252-.164-1.841H9v3.481h4.844a4.14 4.14 0 01-1.796 2.716v2.259h2.908c1.702-1.567 2.684-3.875 2.684-6.615z"
        fill="#4285F4"
      />
      <path
        d="M9 18c2.43 0 4.467-.806 5.956-2.18l-2.908-2.259c-.806.54-1.837.86-3.048.86-2.344 0-4.328-1.584-5.036-3.711H.957v2.332A8.997 8.997 0 009 18z"
        fill="#34A853"
      />
      <path
        d="M3.964 10.71A5.41 5.41 0 013.682 9c0-.593.102-1.17.282-1.71V4.958H.957A8.996 8.996 0 000 9c0 1.452.348 2.827.957 4.042l3.007-2.332z"
        fill="#FBBC05"
      />
      <path
        d="M9 3.58c1.321 0 2.508.454 3.44 1.345l2.582-2.58C13.463.891 11.426 0 9 0A8.997 8.997 0 00.957 4.958L3.964 6.29C4.672 4.163 6.656 3.58 9 3.58z"
        fill="#EA4335"
      />
    </svg>
  );
}
